...
musinsa/data, kurly/data 폴더의 수집 결과(컬리·무신사)를 자동 인식해 카테고리별
...
DATA_DIRS = [Path(__file__).parent / "musinsa" / "data",
             Path(__file__).parent / "kurly" / "data"]
...
def discover_datasets() -> dict[str, dict]:
    """musinsa/data, kurly/data의 products 파일을 (채널·카테고리)별 최신본으로 정리.
    ...
    for key, ppath in latest.items():
        ...
        rev_files = sorted(p for p in ppath.parent.glob(f"{rev_glob}_*.json")
...
def render(label: str, datasets: dict):
    if not label or label not in datasets:
        ...
        return ("`musinsa/data/` · `kurly/data/` 폴더에 수집 파일이 없습니다. "
